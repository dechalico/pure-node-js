...
workers.validateCheckData = (originalCheckData) => {
  ...
  if(originalCheckData.id &&
    ...
    originalCheckData.timeOutSeconds){
      ...
      console.log("Error: Check date is not formatted properly");
    }
};
...
workers.alertUserToStatusChange = (newCheckData) => {
  ...
  helper.sendTwilioSms(newCheckData.userPhone,msg,(err) => {
    if(!err) {
      console.log("Success: User was alerted to a status change thier checks");
      console.log(msg);
    } else {
      console.log("Error: Could not alert the user to SMS aler");
    }
  });
}; 
...
workers.log = (originalCheckData,checkOutcome,state,alert,dateChecked) => {
  ...
  log.append(originalCheckData.id,strJsonData,(err) => {
    if(!err){
      console.log('Data logged succed');
    } else {
        ...
        log.compress(logName,newName,(err) => {
          if(!err) {
            log.truncate(logName,(err) => {
              if(!err){
                console.log('Truncating logFile success');
              } else {
                console.log('Truncating logFile failed');
              }
            });
          } else {
            console.log('Error in compressing the log');
          }
        });
      });
    } else {
      console.log('Could not found any logfile to rotate');
    }
  });
};
