...
const obs = new PerformanceObserver(items => {
  items.getEntries().forEach(p => {
    debug('Name: ' + p.name + ', duration: ' + p.duration);
  });
});
...
handler.exampleError = (data,callback) => {
  const e = new Error('This is a sample error');
  throw e;
};
...
handler._tokens.post = (data,callback) => {
  performance.mark('starting validating')
  const phone = typeof(data.payload.phone) == "string" && 
    data.payload.phone.trim().length == 10 ? data.payload.phone.trim() : false;
  const password = typeof(data.payload.password) == "string" &&
    data.payload.password.trim().length > 0 ? data.payload.password : false;
    performance.mark('ending validating');
  
  if(phone && password){
    performance.mark('starting reading data');
    _data.read('users',phone,(err,data) => {
      performance.mark('ending reading data');
      if(!err && data){
        performance.mark('starting hashing');
        const hashedPassword = helper.hashPassword(password);
        performance.mark('ending hashing');
        if(hashedPassword == data.hashPassword){
          performance.mark('starting creating token');
          ...
          performance.mark('ending creating token');

          performance.mark('starting saving token');
          _data.create('tokens',randomString,tokenData,(err) => {
            performance.mark('ending saving token');

            performance.measure('Start to end','starting validating','ending saving token');
            performance.measure('Start to end reading data','starting reading data','ending reading data');
            performance.measure('Start to end hashing','starting hashing','ending hashing');
            performance.measure('Start to end creating token','starting creating token','ending creating token');
            performance.measure('Start to end saving token','starting saving token','ending saving token');
            ...
            if(!err && userData){
                ...
                if(userChecks.length < config.maxChecks){
                  ...
                  const hostname = typeof(parsedUrl.hostname) == 'string' && 
                    parsedUrl.hostname.trim().length > 0 ? parsedUrl.hostname : false;
                  ...
                  dns.resolve(hostname,(err,result) => {
                    if(!err && result){
                      ...
                      callback(400,{'error': 'Specified url is not accessisable'})
                    }
                  });
