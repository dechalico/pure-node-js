  ...
  req.on('end',() => {
    ...
    try{
      chosenHandler(data,(statusCode,payloadObject,contentType) => {
        server.responseProcessor(res,statusCode,payloadObject,contentType);
      });
    } catch(e){
      server.responseProcessor(res,500,{'error': 'Unknown Internal Error'},'json');
    }
  });
};
...
server.router = {
  ...
  'example/error': handler.exampleError
}
