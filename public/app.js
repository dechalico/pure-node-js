...
app.bindForms = function(){
  if(document.querySelector("form")){
    ...
    for(var i = 0; i < allForms.length; i++){
        ...
        // construct header
        const header = {
          id: typeof(app.config.sessionToken.id) == 'string' ? app.config.sessionToken.id : false
        };

        // construct query string object
        let queryString = {
          phone: typeof(app.config.sessionToken.phone) == 'string' ? app.config.sessionToken.phone : false
        }

        // If the method is DELETE, the payload should be a queryStringObject instead
        queryString = method == 'DELETE' ? payload : {};

        // Call the API
        app.client.request(header,path,method,queryString,payload,function(statusCode,responsePayload){
...
app.loadAccountEditPage = function(){
  ...
  if(phone){
    ...
    app.client.request(header,'api/users','GET',queryStringObject,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
      ...
      if(statusCode == 200){
        ...
        if(allChecks.length > 0){
          ...
          allChecks.forEach(function(checkId){
            ...
            app.client.request(header,'api/checks','GET',newQueryStringObject,undefined,function(statusCode,responsePayload){
              if(statusCode == 200){
...
app.loadChecksEditPage = function(){
  ...
  if(id){
    ...
    app.client.request(header,'api/checks','GET',queryStringObject,undefined,function(statusCode,responsePayload){
      if(statusCode == 200){
